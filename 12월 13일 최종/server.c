	...
	msg_to_client_spe("\n\033[1m현재 생존자: ", who);
        for (i = 0; i < num_user; i++) { // 출력은 i+1로함
			if (user_roles[i] != 0) {
				...
				iplus[0] = i + 49;
				iplus[1] = '\0';
				msg_to_client_spe("\033[33m", who);
				msg_to_client_spe(iplus, who);	//i번째 플레이어
				msg_to_client_spe(" ", who);
				msg_to_client_spe("\033[39m", who);
...
void make_user_roles(int num_user) {
	...
	while (map_count < map) {	//마피아 먼저 정하기
		...
		if (user_roles[temp] == 0) {
			...
			msg_to_client_spe("\033[1;31m",temp);
			msg_to_client_spe("\n\n\t\t당신은 마피아입니다.\n\n", temp);
			msg_to_client_spe("\033[0;39m",temp);
			map_count++;
	...
	while (!pol_count || !doc_count) {	//의사와 경찰 정하기
		...
		if (user_roles[temp] == 0 && pol_count == 0) {
			user_roles[temp] = 2;
			police = temp;
			msg_to_client_spe("\033[1;34m",temp);
			msg_to_client_spe("\n\n\t\t당신은 경찰입니다.\n\n", police);
			msg_to_client_spe("\033[0;39m",temp);
			pol_count = 1;
		}
		...
		if (user_roles[temp] == 0 && doc_count == 0) {
			user_roles[temp] = 3;
			doctor = temp;
			msg_to_client_spe("\033[1;32m",temp);
			msg_to_client_spe("\n\n\t\t당신은 의사입니다.\n\n", doctor);
			msg_to_client_spe("\033[0;39m",temp);
			doc_count = 1;
		}
...
void choice_invest() {			//마피아 알고리즘과 흡사합니다.
	...
	if (user_roles[who_invest] == -1) {	//마피아가 맞는지 아닌지만 경찰에게 알려주는 부분
		...
		msg_to_client_spe("\033[1;33mplayer", police);
		msg_to_client_spe(pol_chat, police);
		msg_to_client_spe("\033[39m은(는) \033[31m마피아\033[39m가 맞습니다.\033[0m\n", police);
		return;
	}
	else {
		...
		msg_to_client_spe("\033[1;33mplayer", police);
		msg_to_client_spe(pol_chat, police);
		msg_to_client_spe("\033[39m은(는) \033[31m마피아\033[39m가 아닙니다..\033[0m\n", police);
		...
		if(read_doc){
			...
			if (who_save >= 0 && who_save < num_user) {	//1~user수 사이의 수를 입력하지 않으면 다시 입력하게함.
				if (!user_roles[who_save]) {
					...
					msg_to_client("\033[1;33m\n");
					msg_to_client_spe(doc_chat, doctor);
					msg_to_client_spe("번 \033[1;39m플레이어를 살립니다.\033[0m\n", doctor);
					break;
				}
			}
			else if(who_save != -1){
...
void night() {
	...
	//밤이되었음을 알림
	msg_to_client("\033[1;34m");
	msg_to_client("\n************밤이 되었습니다.************\n");
	msg_to_client("\033[0;39m");
			...
			if (num_user < 8)
					...
					if(chat[0] != '\n' && chat[1] != 0) //msg to all
					{
...

                                        	{
                                                	if (i != j)
                                                	{
								write(user_sock[j],"\x1b[1;36m", 8);
        	                                                write(user_sock[j], user_name[i], strlen(user_name[i]));
								write(user_sock[j],"\x1b[0;39m", 8);
								write(user_sock[j],"\x1b[1;36m", 8);
                	                                        write(user_sock[j], ">", 2);
								write(user_sock[j],"\x1b[0;39m", 8);
...
void server(int portnum)
{
	...
	msg_to_client("\033[1;32m");
	msg_to_client("\n\n**게임서버에 접속되었습니다.**\n");
	msg_to_client("\033[1;39m");
	...
	//	printf("gameOn: %d\n", gameOn);
		while (gameOn)// if (게임이 시작 되었을 때)
		{
			...
			printf("\033[1;31m현재 마피아 수: %d \t\033[1;36m현재 시민 수: %d\n\033[0;39m", num_mafia, num_civil);
			cur_time = 120;
			while (time_mode == DAY) {
				sleep(1);
				cur_time--;
